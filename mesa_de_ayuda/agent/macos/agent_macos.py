"""
STIC Agent - Main CLI & Service Runner (macOS)
Native launchd background daemon and command-line management tool.
"""
...
from collector_macos import collect_system_data
...
LAUNCHD_PLIST = "/Library/LaunchDaemons/gov.yopal.stic-agent.plist"
...
def setup_logger(log_to_file=True):
    ...
    logger = logging.getLogger("STIC_Agent_macOS")
...
def perform_sync(server_url=None, org_slug=None, api_key=None, proxy=None):
    ...
    logger.info(f"Iniciando recoleccion de inventario macOS para: {org}")
...
def run_daemon():
    ...
    logger.info("STIC Agent iniciado en modo servicio macOS (launchd).")
...
def show_status():
    ...
    print("=" * 50)
    print("   ESTADO DE STIC AGENT (MACOS)")
    print("=" * 50)
    config = load_config()
...
def main():
    parser = argparse.ArgumentParser(description="STIC Agent - macOS Helpdesk Discovery")
